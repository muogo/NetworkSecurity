...
databases = client[DATA_INGESTION_DATABASE_NAME]
collection = databases[DATA_INGESTION_COLLECTION_NAME]
...
@app.get("/train")
async def train_routs():
    try:
        train_pipeline=TrainingPipeline()
        train_pipeline.run_pipeline()
        return Response("Training telah sukses")
...
async def predict_route(request: Request, file: UploadFile = File(...)):
    try:
        ...
        print("Columns in input dataframe:", df.columns.tolist())
        ...
        network_model = NetworlModel(preprocessor=preprocesor,model=final_model)
        print(df.iloc[0])
